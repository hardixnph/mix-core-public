...
public CmdBan(client, level, cid)
{
	...
	{
		console_print(client, "[REPLACE] %L", client, "AB_NOT_AUTHORIZED");
	...
	{
		console_print(client, "[REPLACE] %L", client, "AB_ALREADY_BANNED_STEAMID");
		return PLUGIN_HANDLED;
	}
	#else
	for( new i = 0; i < g_total_bans; i++ )
	...
	{
		console_print(client, "[REPLACE] %L", client, "AB_MAX_BAN_TIME", maxlength);
		return PLUGIN_HANDLED;
	}
	...
	PrintActivity(admin_name, "^x04[REPLACE] $name^x01 :^x03  banned %s. Reason: %s. Ban Length: %s", target_name, arg, unban_time);
...
public CmdBanIp(client, level, cid)
{
	...
	{
		console_print(client, "[REPLACE] %L", client, "AB_ALREADY_BANNED_IP");
		return PLUGIN_HANDLED;
	}
	#else
	for( new i = 0; i < g_total_bans; i++ )
...
public CmdAddBan(client, level, cid)
{
	...
	{
		console_print(client, "[REPLACE] %L", client, "AB_INVALID_STEAMID");
		console_print(client, "[REPLACE] %L", client, "AB_VALID_STEAMID_FORMAT");
	...
	else if( is_ip )
	{
		...
		{
			console_print(client, "[REPLACE] %L", client, "AB_INVALID_IP");
	...
	if( TrieKeyExists(g_trie, target_authid) )
	{
		console_print(client, "[REPLACE] %L", client, is_ip ? "AB_ALREADY_BANNED_IP" : "AB_ALREADY_BANNED_STEAMID");
		return PLUGIN_HANDLED;
	}
	#else
	for( new i = 0; i < g_total_bans; i++ )
	...
	PrintActivity(admin_name, "^x04[REPLACE] $name^x01 :^x03  banned %s %s. Reason: %s. Ban Length: %s", is_ip ? "IP" : "SteamID", target_authid, reason, unban_time);
...
public CmdUnban(client, level, cid)
{
	...
	for( new i = 0; i < g_total_bans; i++ )
	{
		...
		if( equal(arg, banned_authid) )
		{
			...
			PrintActivity(admin_name, "^x04[REPLACE] $name^x01 :^x03  unbanned %s^x01 [%s] [Ban Reason: %s]", name, arg, reason);
	...
	if( TrieKeyExists(g_trie, arg) )
	{
		...
		PrintActivity(unban_name, "^x04[REPLACE] $name^x01 :^x03  unbanned %s^x01 [%s] [Ban Reason: %s]", data[bd_name], data[bd_steamid], data[bd_reason]);
	...
	console_print(client, "[REPLACE] %L", client, "AB_NOT_IN_BAN_LIST", arg);
...
public CmdBanList(client, level, cid)
{
	...
	if( !g_total_bans )
	{
		console_print(client, "[REPLACE] %L", client, "AB_NO_BANS");
		return PLUGIN_HANDLED;
	}
	...
	#endif
	{
		...
		for( new i = 0; i < g_total_bans; i++ )
		{
			...
			|| u_year == c_year && u_month == c_month && u_day == c_day && u_hours == c_hours && u_minutes == c_minutes && u_seconds <= c_seconds )
			{
				...
				Print("^x04[REPLACE]^x03 %s^x01[^x04%s^x01]^x03 ban time is up!^x01 [Ban Reason: %s]", data[bd_name], data[bd_steamid], data[bd_reason]);
				
				RemoveBan(i, data[bd_steamid]);
				#else
				Log("Ban time is up for: %s [%s]", g_names[i], g_steamids[i]);
				
				Print("^x04[REPLACE]^x03 %s^x01[^x04%s^x01]^x03 ban time is up!^x01 [Ban Reason: %s]", g_names[i], g_steamids[i], g_reasons[i]);
